# s21_string+ (Групповой проект. Команда 5 человек)

Реализация библиотеки string.h с дополнениями.

## Description

В данном проекте предстоит разработать собственную реализацию библиотеки string.h на языке программирования Си с некоторыми дополнениями (с собственной реализацией функций sprintf). В рамках этого проекта предполагается отработка задач на работу со строковыми данными и закрепление структурного подхода.
...
## Task. Частичная реализация функции sprintf
...
- *make* собирает полностью проект и провод тесты на корректность
- *make check* стилевая проверка cppcheck и на утечки Valgrind
- *make gcov_report* осуществляется проверка покрытия тестами реализованной функции
- *make clean* очищает каталог от лишних файлов

## Revuies

В результате автотеста не прошли функции strrchr, strncpy, to_upper, trim
